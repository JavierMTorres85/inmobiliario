...
function legend(){const div=document.getElementById('mapLegend');let html='';
 ...
 div.querySelectorAll('[data-legend-bin]').forEach(button=>{button.addEventListener('mouseenter',()=>{legendHover=legendBins()[Number(button.dataset.legendBin)]||null;refreshMapVisuals();});button.addEventListener('click',event=>{event.preventDefault();selectLegendRange(Number(button.dataset.legendBin));});});
 div.querySelector('[data-legend-clear]')?.addEventListener('click',event=>{event.preventDefault();clearLegendRange();});
 div.querySelector('[data-labels-toggle]')?.addEventListener('click',event=>{event.preventDefault();toggleAllLabels();});
}
